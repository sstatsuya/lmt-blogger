  ...
  const runDesc = () => {
    setTimeout(async () => {
      ...
      if (curDescIdx < fullDesc.length - 1) runDesc();
        ...
        if (fullDescIdxRef.current >= descriptions.length)
          fullDescIdxRef.current = 0;
  ...
  const renderTitle = () => {
    return (
      <div className="flex-col flex-center">
        ...
        <h2 className="text-center font-bold text-6xl text-white my-4">
          LMT <span className="text-title underline">BLOG</span>
        </h2>
        <h4 className="text-gray-400 mt-4 font-medium text-lg text-center">
          {currentDescText}
          <span
            className={`text-xl ${
              isBlinkCursor ? "animate-blink" : ""
            } transform -translate-y-0.5 inline-block`}
          >
            |
          </span>
        </h4>
  ...
  const renderAboutMe = () => {
    return (
      <div>
        <SectionTitle
          title="About me"
          desc="Những thông tin cơ bản của tôi, chi tiết hơn nằm trong CV, tôi lười ghi ra quá"
        />
  ...
  const renderProjectList = () => {
    return (
      <div>
        <SectionTitle
          title="Projects"
          desc="Dưới đây là một số dự án cá nhân của tôi"
        />
  ...
  const renderGetInTouch = () => {
    return (
      <div id="get-in-touch">
        ...
        <div className="flex flex-1 flex-col md:flex-row align-middle md:gap-16 gap-8">
          <div className="flex md:w-[30%] flex-col">
            <p className="font-semibold text-white text-lg">
              Thông tin liên hệ
            </p>
            <p className=" text-gray-400 text-sm mt-2">
              Hoặc có thể gửi thư cho tôi bằng cách điền các thông tin ở bảng
              bên phải nhé
            </p>
          ...
          <div className="flex md:w-[70%] flex-col border-1 p-4 border-dim-border rounded-xl">
            ...
            <div className="flex flex-col flex-1 mt-4">
              ...
              <textarea
                rows={5}
                className="py-2 min-h-20 text-sm outline-offset-0 mt-1 px-2 text-gray-400 h-8 border-1 border-dim-border focus:border-gray-500 focus:outline focus:outline-gray-300 focus:outline-1 outline-none rounded-lg bg-transparent"
              />
            </div>
...
const SectionTitle = ({ title, desc }: { title: string; desc: string }) => {
  return (
    <div className="mt-32 mb-8">
      <h1 className="text-4xl text-white font-medium relative inline-block">
        {title}
...
const AboutMeItem = ({ title, data }: { title: string; data: any[] }) => {
  return (
    <div className="border-1 border-dim-border rounded-md p-4 flex-1 flex flex-col gap-4 animate-zoomIn">
      <p className="font-semibold text-white text-xl">
        <LinkedinOutlined
          style={{ fontSize: "24px" }}
          className="text-title mr-4 group-hover:text-title"
        />
        {title}
      </p>
...
const projectLang = ["ReactJS", "NodeJS", "MongoDB", "Rest API"];

const ProjectItem = () => {
  return (
    <div className="border-1 border-dim-border rounded-2xl pb-4 flex-1 flex flex-col gap-4 animate-zoomIn overflow-hidden">
      ...
      <div className="px-4">
        ...
        <div className="gap-4 flex flex-row align-middle mt-4 overflow-auto custom-scroll items-stretch">
          {projectLang.map((item) => {
            return (
              <div className="p-0.5 px-2 text-xs rounded-lg bg-title flex-center">
