...
import { Bold, Italic, Strikethrough, Baseline, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, Code2, Undo2, Redo2, LinkIcon, Eye, EyeOff } from 'lucide-react'
...
const CreatePost = () => {
    ...
    const renderTitle = () => {
        return <div className="w-full">
            <input placeholder="Tiêu đề" className='text-base w-full outline-offset-0 mt-1 px-2 text-white h-12 border-1 border-dim-border focus:border-gray-500 focus:outline focus:outline-gray-300 focus:outline-1 outline-none rounded-lg bg-gray-700' />
        </div>
    }

    const renderRichText = () => {
        if (!editor) return null
        return <div className="w-full mt-4 border-1 border-dim-border rounded-lg bg-gray-700">
            {/* Toolbar */}
            <div className="flex gap-2 border-b border-dim-border px-3 py-2">
                ...
                >
                    <Bold color={`${editor.isActive('bold') ? 'black' : 'white'}`} size={18} />
                </button>
                <button
                ...
                >
                    <Italic color={`${editor.isActive('italic') ? 'black' : 'white'}`} size={18} />
                </button>
                <button
                ...
                >
                    <Strikethrough color={`${editor.isActive('strike') ? 'black' : 'white'}`} size={18} />
                </button>

                <button
                    onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}
                    className={`p-1 rounded hover:bg-gray-200 ${editor.isActive('heading', { level: 1 }) ? 'bg-gray-300 text-black' : 'text-white'}`}
                >
                    H1
                </button>
                ...
                <button
                    ...
                    className={`p-1 rounded hover:bg-gray-200 ${editor.isActive('heading', { level: 2 }) ? 'bg-gray-300 text-black' : 'text-white'}`}
                >
                    H2
                </button>
                ...
                <button
                    ...
                    className={`p-1 rounded hover:bg-gray-200 ${editor.isActive('heading', { level: 3 }) ? 'bg-gray-300 text-black' : 'text-white'}`}
                >
                    H3
                </button>
                ...
                >
                    <List color={`${editor.isActive('bulletList') ? 'black' : 'white'}`} size={22} className='' />
                </button>
                ...
                >
                    <ListOrdered color={`${editor.isActive('orderedList') ? 'black' : 'white'}`} size={22} className='' />
                </button>
                ...
                >
                    <AlignLeft color={editor.isActive({ textAlign: 'left' }) ? 'black' : 'white'} size={18} />
                </button>
                ...
                >
                    <AlignCenter color={editor.isActive({ textAlign: 'center' }) ? 'black' : 'white'} size={18} />
                </button>
                ...
                >
                    <AlignRight color={editor.isActive({ textAlign: 'right' }) ? 'black' : 'white'} size={18} />
                </button>
                ...
                >
                    <Code2 color={editor.isActive('codeBlock') ? 'black' : 'white'} size={18} />
                </button>
                ...
                <button
                    ...
                    className="p-1 rounded hover:bg-gray-200"
                >
                    <LinkIcon size={18} color={editor.isActive('link') ? 'black' : 'white'} />
                </button>
            ...
            <EditorContent
                editor={editor}
                className="p-4 w-full prose prose-invert"
                style={{ outline: 'none' }}
            />
        </div>
    }
    ...
    const renderSubmit = () => {
        return <>
            <div onClick={() => onSubmit()} className='w-full py-2 bg-title text-white font-semibold text-center rounded-lg mt-4 cursor-pointer hover:opacity-80 transition-all duration-500'>
                Xuất bản
            </div>
            <div onClick={() => setIsPreview(!isPreview)} className='flex flex-center gap-1 w-full py-2 bg-title text-white font-semibold text-center rounded-lg mt-4 cursor-pointer hover:opacity-80 transition-all duration-500'>
                <span>Xem trước</span> <>
                    {isPreview ? (
