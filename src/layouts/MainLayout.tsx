...
const MainLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const renderUtilities = () => {
    return <div className='flex-between gap-8'>
      ...
      <div onClick={() => navigate(APP_ROUTE.POST)} className={`cursor-pointer ${location.pathname === APP_ROUTE.POST ? 'text-title' : 'text-white'} hover:text-title animate-transition`}>
        Bài viết
      </div>
  ...
  const renderFooter = () => {
    return <div className='w-full bg-footer p-8 text-center'>
  ...
  return (
    <div className={'flex flex-col'}>
      <div className={`flex flex-col min-h-screen relative ${isSignInPage ? '' : 'xl:px-[20%] px-[5%]'}`}>
        {!isSignInPage && <>
          <div id="progress-bar" className='fixed top-0 left-0 w-full h-1 bg-transparent z-30'>
            <div id="progress-fill" className="h-full bg-title" style={{ width: '0%' }}></div>
          </div>
